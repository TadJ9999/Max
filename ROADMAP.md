...
- [x] **Quantization / KV-cache / context-length tuning** — `TuningConfig` (`config.py`: `num_ctx` / `num_gpu` / `num_batch` / `num_thread`, 0/-1 = Ollama default) round-trips through `/config`; `factory.py` passes `tuning.to_options()` into `OllamaProvider`, which merges them into every local chat's `options` (per-call params still override). Model-quant is the model tag; KV-cache quant is the `OLLAMA_KV_CACHE_TYPE` server env (both surfaced in the UI hint). **Ollama tuning card** in Model Manager (Local tab)
- [x] **Latency targets / measurement tooling** — `probe_latency()` (`models/benchmark.py`) fires a tiny capped prompt N× and reports **median TTFT + median end-to-end** (isolating latency from throughput); `POST /models/latency` stores it in `.apollo.db` (`lat_ttft_ms`/`lat_total_ms`, disjoint from benchmark columns); `/models` surfaces it and Model Manager cards show **TTFT + latency** next to tok/s with a "Probe latency" button
...
- [x] **More providers + more clients** — OpenAI provider built (`%` sigil, `providers/openai_provider.py`, cost catalog) + LAN browser client (Phase 17) + **OpenAI-compatible local adapter** (`^` sigil, `providers/openai_compat.py`): one `local`-kind provider for any local server speaking `/v1/chat/completions` (llama.cpp `llama-server`, vLLM, LM Studio); connect-only (no spawning), no API key, no egress, usage recorded at $0 like Ollama; configurable `base_url` (default `:8080`) round-tripped through `/config`; Model Manager "Local server" card (reachability dot + served models from `GET /v1/models`) + Task Routing optgroup; 6 tests. **Neovim client built** (`clients/nvim/`): pure-Lua plugin (zero deps, `curl` via `vim.system`), engine unchanged — DSL run + inline-replace streaming, auto-trigger on closing delimiter, FIM ghost-text (`/complete`, inline virt-text + `<Tab>`), visual-selection ops, statusline health/cloud indicator; sigils `@ # ! % ^`; `:Max`/`:MaxHealth`/`:MaxToggleAutocomplete`; headless pure-function tests (`nvim --headless -l test/run.lua`). **CLI client built** (`engine/max_engine/cli.py`, `max` console script in `pyproject.toml`, [`clients/cli/`](clients/cli/) README): pure-httpx (no new deps) terminal client — one-shot (`max "!. … ."` → `/command`, auto-falls back to `/chat` on a non-command), interactive multi-turn REPL (memory via `/v1/chat/completions`; DSL lines run one-shot), `max health`, `max sessions [cancel|promote] <id>`; `--engine` / `MAX_ENGINE_URL` so it drives a local **or remote** engine; ANSI styling with `NO_COLOR` opt-out
...
- [x] *(stretch)* **Stream diagnosis token-by-token** — `Invoke-ClaudeDiagnosisStream` / `Invoke-OllamaDiagnosisStream` in `scripts/leo.ps1` read the SSE/JSONL body via `HttpClient` (ResponseHeadersRead) and print tokens live in red as they arrive; **one-click "apply suggested commands"** — `Get-FixCommands` extracts runnable commands from the diagnosis, `[A] Apply fix` shows + confirms + runs them then re-checks `/health`. **Cross-platform launcher built** ([`Max.command`](Max.command)): macOS/Linux **thin-client** launcher — resolves the remote engine URL (`MAX_ENGINE_URL` → `client.remote_url` in `.maxconfig.json` → default), persists it, health-checks `GET /health` (friendly checklist on failure instead of a broken window), then builds (first run) + launches the app pointed at the Windows engine. UI here, all compute on the PC; see [docs/lan.md](docs/lan.md)
...
- [x] *(stretch)* **Rust/Tauri auto-fix path** — `aegis_auto_fix(event_id)` Tauri command (`lib.rs`) POSTs `/aegis/auto-fix/{id}` over raw TCP and returns the collected SSE body; `AegisView` drives the desktop Auto-Fix through this native path (`autoFixNative` in `aegis.ts`), falling back to the in-webview SSE stream in the browser/LAN client. Streaming token-by-token Leo diagnosis: done (see Leo). **Cross-platform launcher built** — [`Max.command`](Max.command) thin-client launcher for macOS/Linux (see Leo section + [docs/lan.md](docs/lan.md)); the Tauri app runs in **remote-engine mode** on non-Windows (`read_remote_engine()` → `engine_base()` returns the remote URL and `spawn_engine()` no-ops; `get_remote_engine`/`set_remote_engine` commands)
...
### Phase 19 — Consumer Claude: subscription-backed agent in the editor (`#` sigil)  🎯 *use your Claude Max subscription, not the API key — Opus in the editor with no per-token bill*

*Adds a second Claude rail alongside the API-key path. The Messages API can only bill an API key, so the subscription is used the only way it can be — by spawning the logged-in `claude` CLI in headless mode (`claude -p`). A new `ClaudeAgentProvider` conforms to the existing `Provider.chat` interface, so it drops into `_sse_stream` + `build_provider` with **zero editor changes** — the inline command already routes by sigil. `!` stays on the API key (per-token Opus); `#` runs on the subscription. Phase 19a is safe single-shot text; 19b turns the same runner into a file-editing, multi-instance agent.*

**Decisions locked:** subscription reached via headless `claude -p` subprocess (not Messages API) · **child env strips `ANTHROPIC_API_KEY`/`AUTH_TOKEN`/`BASE_URL`** so it bills the subscription, not the key · `#` sigil → `claude-cli` provider, qwen relocates to `q` · per-command rail choice (sigil), not a global mode toggle · `!` (API key) and `#` (subscription) coexist · default models = Opus for generate/fix/chat, Sonnet for summarize (configurable in Model Manager) · binary auto-detected (not on PATH) with config override · gated by `allow_cloud` / `force_offline` like other off-machine calls · subprocess pattern reuses the proven `MCPStdioClient` approach · stdlib-only (no Agent-SDK dependency).

**Prerequisites (verified 2026-06-01):** ✅ standalone CLI installed at `~/.local/bin/claude.exe` and logged in on the Max subscription (the desktop-app Store binary's session is NOT visible to spawned processes — needed a separate `claude /login`) · ✅ env-strip confirmed: a headless spawn reports `apiKeySource: none` + a `five_hour` rate-limit event (subscription quota, not API billing).

**Phase 19a — single-shot text agent (safe; no file edits)  ✅ BUILT & e2e-verified** (`#. Reply with: pong .` → spawns the real `claude.exe` → streams `pong`, no API key)
- [x] **`ClaudeAgentProvider`** (`engine/max_engine/providers/claude_agent.py`): `kind="agent"`; spawns `claude --print --input-format text --output-format stream-json --include-partial-messages --verbose --model <alias> --max-turns 1 --disallowed-tools Edit Write Bash NotebookEdit` via `create_subprocess_exec`; prompt over **stdin** (dodges Windows argv limit); system turn → `--system-prompt` (**replaces** the heavy default agent prompt → lean quota + plain-assistant behaviour); parses `stream_event → text_delta` deltas (falls back to `assistant` message, then `result`); strips `ANTHROPIC_API_KEY`/`AUTH_TOKEN`/`BASE_URL` from child env; egress log + usage callback mirroring `anthropic.py`; injectable `spawn` for tests; `find_claude()` detection (config → `which` → `~/.local/bin` → npm `.cmd` shim wrapped in `cmd /c`).
- [x] **Factory** (`providers/factory.py`): branch `kind=="agent"` → `ClaudeAgentProvider(claude_path=config.claude_cli_path)`.
- [x] **Config** (`config.py`): `sigils` `#`→`claude-cli`, `q`→`qwen`; `ProviderConfig(name="claude-cli", kind="agent")`; `provider_models["claude-cli"]` (Opus/Sonnet); new UI-editable `claude_cli_path` override.
- [x] **Parser** (`dsl/parser.py`): `DEFAULT_SIGILS` `#`→`claude-cli`, `q`→`qwen`; `claude-cli` added to `cloud_providers` (gating + confirm UX); docstring sigil table updated.
- [x] **Router/main** (`router.py`, `main.py`): `is_cloud_provider` treats `agent` kind as off-machine (gated by `allow_cloud`); `/command` now enforces `force_offline` on cloud/agent routes (subprocess bypasses the httpx guard); usage callback registered at startup.
- [x] **Frontend** (`app/src/engine.ts`): `isDslCommand`/`isDslCommandExtended` sigil set now includes `q`; (`app/src/code/CodeView.tsx`) empty-state hint mentions `#` = subscription Claude, `!` = Claude API.
- [x] **Tests** (`engine/tests/test_claude_agent.py`, 13 cases + `test_parser.py`): injected fake spawn — ordered deltas, fallbacks, `done` on `result`, **child env has no API key**, system→`--system-prompt`, model-alias + single-shot guards, not-logged-in/non-zero-exit raise, `find_claude` precedence; parser `#`→claude-cli (`is_cloud`) / `q`→qwen. **412 passed, tsc clean.**

**Phase 19b — full agentic mode (edits files, multiple instances)**
- [ ] **Agentic spawn**: drop `--max-turns 1`/tool ban; add `--add-dir <workspace_allowlist>` + `--permission-mode acceptEdits|plan`; stream tool-use/edit events as status; surface diffs via the Code tab's existing git-snapshot rollback.
- [ ] **Guardrails**: confine to `workspace_allowlist`; honor `force_offline`; `max_concurrent_agents` cap.
- [ ] **Multi-instance**: route `delegate/coordinator.py` workers to the `claude-cli` provider — each parallel subtask = its own `claude` process.
- [ ] **Endpoint**: `POST /agent` in `main.py` (same SSE shape) carrying workspace + permission options the bare sigil can't express; `#` stays the quick single-shot path.
